...
describe("Gilgamesh", () => {
	it("uses the configured base-speed rule and locks 兴致 as the first resource", () => {
		const gil = character("gil", "吉尔伽美什", 100, {
			lc_id: 23044,
			superimpose: 3,
		});
		...
		expect(
			normalizeResourcesForCharacters(["战技点", "自定义"], [gil]),
		).toEqual(["兴致", "战技点", "自定义"]);
	});
	...
	it("uses AV0 manual 兴致, accumulates actions and permanently changes the lock at 10", () => {
		...
		expect(
			actions.find((action) => action.key === "@av0-1")?.gilgameshInterest,
		).toBe(10);
	...
	it("technique adds three 兴致 on entry, including on top of E2", () => {
		const e0 = simulateActions(
			input({
				characters: [
					character("gil", "吉尔伽美什", 100, { techniqueOn: true }),
				],
				limit: 1,
			}),
		);
		const e2 = simulateActions(
			input({
				characters: [
		...
		expect(
			e0.find((action) => action.key === "@av0-1")?.gilgameshInterest,
		).toBe(3);
		expect(
			e2.find((action) => action.key === "@av0-1")?.gilgameshInterest,
		).toBe(8);
	});
	...
	it("technique action preserves the configured display name", () => {
		const actions = simulateActions(
			input({
				characters: [
					{
		...
		expect(
			actions.find((action) => action.isGilgameshTechniqueAction)?.displayName,
		).toBe("闪");
	});
	...
	it("applies Q and per-row manual 兴致 values as the following baseline", () => {
		...
		expect(
			actions.find((action) => action.key === "ally-1")?.gilgameshInterest,
		).toBe(3);
		expect(
	...
	it("E2 starts at 5 and its Q does not clear 兴致 before adding its two-plus-five bonus", () => {
		const actions = simulateActions(
			input({
				characters: [
					character("gil", "吉尔伽美什", 100, { eidolon: 2 }),
					character("ally", "停云", 200),
				],
		...
		expect(
			actions.find((action) => action.key === "ally-1-interrupt-0")
				?.gilgameshInterest,
		).toBe(13);
	});
	...
	it("counts Phainon domain EA/EW as exactly two 兴致, not one plus two", () => {
		...
		expect(
			actions.find((action) => action.key === "phainon-1-domain-0"),
		).toMatchObject({
			skill: "EW",
			gilgameshInterest: 6,
		});
	...
	it("counts one Aha moment by its two Elation participants only once", () => {
		...
		const elationSkills = actions.filter(
			(action) =>
				action.isElationSkill && action.elationSkillParentKey === aha?.key,
		);
	...
	it("does not gain 兴致 from an Aha moment occurring inside Phainon domain", () => {
		...
		const aha = actions.find((action) => action.isAhaInstant);
		expect(
			actions.find((action) => action.key === "phainon-1-domain-1"),
		).toMatchObject({
			gilgameshInterest: 6,
		});
