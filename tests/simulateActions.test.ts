...
describe("simulateActions", () => {
    ...
    it("uses positive speed adjustment as acceleration and negative as slowdown", () => {
        ...
        expect(actions.find((action) => action.key === "c1-1")?.actionValue)
            .toBeCloseTo(100, 4);
        expect(actions.find((action) => action.key === "c1-2")?.actionValue)
            .toBeCloseTo(166.6667, 4);
        expect(actions.find((action) => action.key === "c1-3")?.actionValue)
            .toBeCloseTo(266.6667, 4);
    });

    it("applies 24 percent team advance for Dance Dance Dance ultimates", () => {
        const actions = simulateActions(
            input({
                characters: [
                    character("a", "停云", 100, {
        ...
        expect(actions.map((action) => action.key).slice(0, 2)).toEqual([
            "a-1",
            "b-1",
        ]);
        expect(actions.find((action) => action.key === "b-1")?.actionValue)
            .toBeCloseTo(100, 4);
    });
    ...
    it("activates Aglaea Supreme Stance from an interrupt ultimate", () => {
        ...
        expect(actions.find((action) => action.key === "aglaea-1"))
            .toMatchObject({
                isAglaeaSupremeAction: true,
                actionValue: 100,
            });
...
describe("canUseSkillCode", () => {
    const char = (name: string, opts?: { hasW?: boolean; wDomainOnly?: boolean }): CharacterConfig => ({
        id: name, kind: "角色", name,
        speed: "100", baseSpeed: "100",
        hasVonwacq: false, hasWindSet: false, hasDance: false,
        hasEidolon1: false, hasEidolon2: false, hasEidolon4: false,
    });
    ...
    it("允许标准技能", () => {
        ...
        expect(canUseSkillCode(char("任意角色"), "Q")).toBe(true);
    });
...
describe("team advance 24%", () => {
    it("忘归人 E2 Q 后全队 24% 拉条", () => {
        ...
        const actions = simulateActions(
            input({
                characters: [
                    character("fugue", "忘归人", 150, { hasEidolon2: true }),
                    character("a", "行动角色", 100),
                ],
                skillOverrides: skills({ "fugue-1": "Q" }),
                limit: 150,
            }),
        );
    ...
    it("舞舞舞 Harmony Q 后全队 24% 拉条", () => {
        // 停云（Harmony）装备舞舞舞，Q 后全队 24% 拉条
        const actions = simulateActions(
            input({
                characters: [
                ...
                skillOverrides: skills({ "tingyun-1": "Q" }),
                limit: 150,
            }),
        );
        const teamAV = actions.filter((a) => a.key === "a-1")[0]?.actionValue;
    ...
    it("非 Harmony 角色舞舞舞无效", () => {
        ...
        const actions = simulateActions(
            input({
                ...
                skillOverrides: skills({ "seele-1": "Q" }),
                limit: 150,
            }),
        );
        const teamAV = actions.filter((a) => a.key === "a-1")[0]?.actionValue;
    ...
    it("知更鸟 Q 后全队顶轴排列", () => {
        const actions = simulateActions(
            input({
                ...
                skillOverrides: skills({ "robin-1": "Q" }),
                limit: 200,
            }),
        );
        ...
        const afterQ = actions.filter((a) => a.actionValue >= 37);
...
describe("first action advance", () => {
    it("翁瓦克 40% 首动提前", () => {
        const actions = simulateActions(
            input({
                characters: [
                    character("a", "行动角色", 100, { hasVonwacq: true }),
                ],
                limit: 150,
            }),
        );
...
describe("wind set", () => {
    it("风套 Q 后 25% 行动提前", () => {
        const actions = simulateActions(
            input({
                characters: [
                    character("a", "行动角色", 100, { hasWindSet: true }),
                ],
                skillOverrides: skills({ "a-1": "Q" }),
                limit: 250,
            }),
        );
...
describe("himeko nova F assist", () => {
    it("队友 FE 行动触发协战", () => {
        ...
        // a-1（FE）在 100 AV 行动，应触发姬子的 F 协战
