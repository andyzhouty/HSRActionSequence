...
describe("ActionPanel RMC / Evernight", () => {
	...
	it("Sunday 的 E 目标列表不会出现死龙、衣匠、迷迷、长夜", async () => {
		...
		const chars = [
			...
			{ id: "evernight", name: "长夜月", kind: "角色" as const, speed: "100", baseSpeed: "100", hasVonwacq: false, hasWindSet: false, hasDance: false, eidolon: 0, superimpose: 1, lc_id: 0 },
		];
		const memos = [
			...
			{ id: "evernight-evey", name: "长夜", kind: "忆灵" as const, speed: "160", baseSpeed: "160", hasVonwacq: false, hasWindSet: false, hasDance: false, eidolon: 0, superimpose: 1, lc_id: 0 },
		];
		renderWithContext(<ActionPanel />, {
			characters: chars,
			characterNames: {
	...
	it("敌人回合右键菜单会显示长夜自爆开关", () => {
		...
		const chars = [
			{ id: "evernight", name: "长夜月", kind: "角色" as const, speed: "100", baseSpeed: "100", hasVonwacq: false, hasWindSet: false, hasDance: false, eidolon: 0, superimpose: 1, lc_id: 0 },
			{ id: "enemy", name: "敌人", kind: "敌人" as const, speed: "200", baseSpeed: "200", hasVonwacq: false, hasWindSet: false, hasDance: false, eidolon: 0, superimpose: 1, lc_id: 0 },
		];
		renderWithContext(<ActionPanel />, {
			characters: chars,
			characterNames: { evernight: "长夜月", enemy: "敌人", "evernight-evey": "长夜" },
			characterKinds: { evernight: "角色", enemy: "敌人", "evernight-evey": "忆灵" },
			charactersById: {
				evernight: chars[0],
				enemy: chars[1],
				"evernight-evey": { id: "evernight-evey", name: "长夜", kind: "忆灵" as const, speed: "160", baseSpeed: "160", hasVonwacq: false, hasWindSet: false, hasDance: false, eidolon: 0, superimpose: 1, lc_id: 0 },
			},
			actions: [enemyAction],
			actionMenuOpen: true,
			actionMenuKey: "enemy-1",
			selectedActionKeys: new Set(["enemy-1"]),
			memospriteTargets: [{ id: "evernight-evey", name: "长夜", kind: "忆灵" as const, speed: "160", baseSpeed: "160", hasVonwacq: false, hasWindSet: false, hasDance: false, eidolon: 0, superimpose: 1, lc_id: 0 }],
		});

		expect(screen.getByText("长夜自爆：")).toBeInTheDocument();
		expect(screen.getByText("未填写忆质时，可通过右键手动标记自爆")).toBeInTheDocument();
	});
	...
	it("填写的忆质小于16时，右键菜单会提示本次不会自爆", () => {
		...
		expect(screen.getByText("长夜自爆：")).toBeInTheDocument();
		expect(screen.getByText("当前忆质 < 16，视为误操作，本次不会自爆")).toBeInTheDocument();
	});
	...
	it("填写的忆质大于等于16时，右键菜单会显示已自爆", () => {
		...
		expect(screen.getByText("当前忆质 >= 16，长夜会在本回合结束后自动以锁定 E 立刻行动并离场")).toBeInTheDocument();
	});
