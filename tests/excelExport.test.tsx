 ...
 *
 * Tests domain filtering, row generation, character sheet generation,
 * and workbook structure. Does NOT test actual file writing (needs Wails).
 *
 * Run manually: npx vitest run tests/excelExport.test.tsx
 */
...
function buildDomainRanges(actions: MinimalAction[]): DomainRange[] {
	...
	for (const a of actions) {
		if (a.isDomainAction && !a.isDomainFinalAction) {
			...
			if (!existing) {
				ranges.push({ casterId: a.characterId, startAV: a.actionValue, endAV: 0 });
			}
		}
...
describe("Excel export: domain filtering", () => {
	...
	it("keeps domain actions", () => {
		const actions: MinimalAction[] = [
			{ key: "p-1", characterId: "phainon", actionNo: 1, actionValue: 100, skill: "Q", isDomainAction: false },
			{ key: "p-domain-0", characterId: "phainon", actionNo: 1, actionValue: 100, skill: "", isDomainAction: true },
			{ key: "p-domain-1", characterId: "phainon", actionNo: 2, actionValue: 120, skill: "", isDomainAction: true, isDomainFinalAction: true },
		];
	...
	it("hides ally actions inside domain range", () => {
		const actions: MinimalAction[] = [
			{ key: "p-1", characterId: "phainon", actionNo: 1, actionValue: 100, skill: "Q" },
			{ key: "p-domain-0", characterId: "phainon", actionNo: 1, actionValue: 100, skill: "", isDomainAction: true },
			{ key: "p-domain-1", characterId: "phainon", actionNo: 2, actionValue: 200, skill: "Q", isDomainAction: true, isDomainFinalAction: true },
			{ key: "s-1", characterId: "sparkle", actionNo: 1, actionValue: 150, skill: "E" }, // inside domain
		];
		const ranges = buildDomainRanges(actions);
	...
	it("keeps enemy actions inside domain", () => {
		const actions: MinimalAction[] = [
			...
			{ key: "p-domain-0", characterId: "phainon", actionNo: 1, actionValue: 100, skill: "", isDomainAction: true },
			{ key: "p-domain-1", characterId: "phainon", actionNo: 2, actionValue: 200, skill: "Q", isDomainAction: true, isDomainFinalAction: true },
			{ key: "e-1", characterId: "enemy", actionNo: 1, actionValue: 150, skill: "" },
		];
		const ranges = buildDomainRanges(actions);
	...
	it("keeps Aha actions inside domain", () => {
		const actions: MinimalAction[] = [
			...
			{ key: "p-domain-0", characterId: "phainon", actionNo: 1, actionValue: 100, skill: "", isDomainAction: true },
			{ key: "p-domain-1", characterId: "phainon", actionNo: 2, actionValue: 200, skill: "Q", isDomainAction: true, isDomainFinalAction: true },
			{ key: "aha-1", characterId: "aha", actionNo: 1, actionValue: 150, skill: "", isAhaInstant: true },
		];
		const ranges = buildDomainRanges(actions);
		expect(isInsideDomain(actions[3], ranges, kinds)).toBe(false);
	});
	...
	it("keeps ally actions outside domain range", () => {
		const actions: MinimalAction[] = [
			...
			{ key: "p-domain-0", characterId: "phainon", actionNo: 1, actionValue: 100, skill: "", isDomainAction: true },
			{ key: "p-domain-1", characterId: "phainon", actionNo: 2, actionValue: 200, skill: "Q", isDomainAction: true, isDomainFinalAction: true },
			{ key: "s-1", characterId: "sparkle", actionNo: 1, actionValue: 50, skill: "E" }, // before domain
			{ key: "s-2", characterId: "sparkle", actionNo: 2, actionValue: 250, skill: "E" }, // after domain
		];
		const ranges = buildDomainRanges(actions);
	...
	it("keeps domain caster's own actions", () => {
		const actions: MinimalAction[] = [
			...
			{ key: "p-domain-0", characterId: "phainon", actionNo: 1, actionValue: 100, skill: "", isDomainAction: true },
			{ key: "p-domain-1", characterId: "phainon", actionNo: 2, actionValue: 200, skill: "Q", isDomainAction: true, isDomainFinalAction: true },
			{ key: "p-2", characterId: "phainon", actionNo: 2, actionValue: 150, skill: "E" }, // Phainon's own action inside his domain
		];
		const ranges = buildDomainRanges(actions);
		expect(isInsideDomain(actions[3], ranges, kinds)).toBe(false);
	});
	...
	it("handles multiple domain ranges", () => {
		...
		const actions: MinimalAction[] = [
			{ key: "p1-1", characterId: "p1", actionNo: 1, actionValue: 100, skill: "Q" },
			{ key: "p1-domain-0", characterId: "p1", actionNo: 1, actionValue: 100, skill: "", isDomainAction: true },
			{ key: "p1-domain-1", characterId: "p1", actionNo: 2, actionValue: 200, skill: "Q", isDomainAction: true, isDomainFinalAction: true },
			{ key: "p2-1", characterId: "p2", actionNo: 1, actionValue: 300, skill: "Q" },
			{ key: "p2-domain-0", characterId: "p2", actionNo: 1, actionValue: 300, skill: "", isDomainAction: true },
			{ key: "p2-domain-1", characterId: "p2", actionNo: 2, actionValue: 400, skill: "Q", isDomainAction: true, isDomainFinalAction: true },
			{ key: "s-1", characterId: "sparkle", actionNo: 1, actionValue: 150, skill: "E" }, // inside p1 domain, outside p2
		];
		const ranges = buildDomainRanges(actions);
