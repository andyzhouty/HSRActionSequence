...
import { getEffectiveCharacterBaseSpeed } from "../../src/mechanics/lightconeEffects";
...
describe("光锥 23063 你将起身歌唱", () => {
	it("装备者首动提前 40%，全队速度 +20%", () => {
		...
		// robin: 被动 20% + 光锥 40% → 首动 = 10000×0.4/100 = 40，再被 +20% 全队加速重排 → 40×100/119.6
		const robinFirst = actions.find((action) => action.key === "robin-1");
		expect(robinFirst).toBeDefined();
		expect(robinFirst?.actionValue).toBeCloseTo((40 * 100) / 119.6, 1);
		expect(robinFirst?.speed).toBeCloseTo(119.6, 2);
	...
	it("全队加速持续 2 个正常回合后移除", () => {
		...
		// robin: 第 1/2 动速度 119.6，第 3 动回到 100
		const robinActions = actions.filter(
			(action) => action.characterId === "robin",
		);
		expect(robinActions[0].speed).toBeCloseTo(119.6, 2);
		expect(robinActions[1].speed).toBeCloseTo(119.6, 2);
...
describe("光锥 23064 向浪花掷下盛夏", () => {
	...
	it("装备者施放欢愉技后速度提高 20%（基于基础速度）", () => {
		...
		// 阿哈时刻（100 速）触发水砂欢愉技后：100 + 107×0.2 = 121.4
		const spavenSecond = acts.find((a) => a.key === "spaven-2");
		expect(spavenSecond).toBeDefined();
		expect(spavenSecond?.speed).toBeCloseTo(100 + 107 * 0.2, 2);
	});
});
