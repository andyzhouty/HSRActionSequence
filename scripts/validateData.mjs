...
const data = JSON.parse(await readFile(characterFile, "utf8"));
...
if (!Array.isArray(characters)) {
	...
	const visit = (value, path) => {
		...
		for (const [key, nested] of Object.entries(value)) {
			...
			if (key === "targetCid" && (!knownCids.has(nested) || typeof nested !== "string")) {
...
if (errors.length > 0) {
	...
	console.log(`数据校验通过：${characters.length} 个角色`);
}
