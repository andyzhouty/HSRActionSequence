...
import type { SimulateActionsInput } from "./simulateActions";
...
function applyCountdownDelay(
    states: FireflyActionState[],
    stateIndex: number,
) {
    const rule = getFireflyCombustionRule(
        states[stateIndex].character.name,
    );
    ...
    if (countdown) {
        ...
        countdown.nextActionValue =
            ...
            delayPerBreak *
                Math.min(rule.maxBreakDelayTriggers, totalBreaks);
    }
}
