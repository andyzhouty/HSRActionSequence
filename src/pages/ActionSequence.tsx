...
function clampOverridesToDisplayedLimit(
    ...
    return Object.fromEntries(
        Object.entries(overrides).map(([key, value]) => {
            ...
            return [
                key,
                formatEditableNumber(Math.min(parsed, displayedLimit)),
            ];
        }),
    );
}
...
export default function ActionSequence() {
    ...
    >({});
    const [memeSelections, setMemeSelections] = useState<
        Record<string, string>
    >({});
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                const savedDisplayedActionLimit =
                    getSavedDisplayedActionLimit(parsed);
                    ...
                    parsed.characters.map((character, index) =>
                        withoutCharacterOnlyEffects({
                            ...createTarget(
                                ...
                                index,
                                (targetKinds.includes(
                                    character.kind as TargetKind,
                                )
                                    ? character.kind
                                    : "角色") as TargetKind,
                            ),
                ...
                setLimitPreset(
                    parsed.limitPreset &&
                        limitPresets.includes(parsed.limitPreset)
                        ? parsed.limitPreset
                        : "150",
                );
    ...
    const updateActionSkill = (action: GeneratedAction, value: string) => {
        ...
        if (action.isAglaeaSupremeAction) {
            const garmentmakerRule = getGarmentmakerRule(character.name);
            const allowedSupremeSkills = new Set(
                garmentmakerRule.allowedSupremeSkills,
            );
            if (!allowedSupremeSkills.has(nextSkill)) {
                setMessage(
                    `阿格莱雅至高之姿下只能填写 ${garmentmakerRule.allowedSupremeSkills
                        .filter(Boolean)
                        .join("、")}`,
                );
                return;
        ...
        if (!action.isDomainAction && !canUseSkillCode(character, nextSkill)) {
            if (nextSkill.includes("A") && nextSkill.includes("E")) {
                ...
                const ownerNames =
                    getSkillEffectOwnerNames("W", "counterW").join("、") ||
                    "对应角色";
        ...
        try {
            ...
            const resourceColWidth =
                resources.length > 0
                    ...
                    : 0;
            const compactWidth =
                fixedColWidth + resourceColWidth * resources.length;
            const dataUrl = await domToPng(target, {
                ...
                onCloneNode: (cloned) => {
                    ...
                    cloned.querySelectorAll("input").forEach((input) => {
                        ...
                        const isSkillInput =
                            inputElement.classList.contains("font-bold");
                        const isActionValue =
                            inputElement.classList.contains("font-mono") &&
                            !isSkillInput;
                        const isDomainSkill =
                        ...
                        replacement.style.color = isDomainSkill
                            ? "#fef3c7"
                            : "#ffffff";
                        replacement.style.display = "flex";
                        replacement.style.fontSize = isSkillInput
                            ? "0.875rem"
                            : "inherit";
