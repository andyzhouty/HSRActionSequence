    ...
    const doExport = useCallback(async () => {
        ...
        try {
            ...
            const rows = ctx.actions.map((a, i) => {
                ...
                if (odeSelection) {
                    const odeTargetName =
                        ctx.characterNames[odeSelection.targetId] ??
                ...
                const row: (string | number)[] = [
                    i + 1,
                    a.displayName ??
                        ctx.characterNames[a.characterId] ??
                        a.characterId,
                    a.isDomainAction
                        ? `境界${a.actionNo}`
                        : `第${a.actionNo}动`,
            ...
            const actionLabel = (key: string, charId: string): string => {
                ...
                if (interruptMatch)
                    return `插队#${Number(interruptMatch[1]) + 1}`;
                ...
                return [
                    ...
                    formatEntries(interruptEntries, c.id, (arr) =>
                        (arr as UltInterrupt[])
                            .map(
                                (u) =>
                                    `${u.casterId}(${u.timing === "before" ? "前" : "后"})`,
                            )
                            .join(","),
                    ),
                ];
