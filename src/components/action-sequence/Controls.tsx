...
export function NumberInput({
    ...
    return (
        <label className="block">
            <span
                className={`mb-1 block text-sm text-gray-300 ${labelClassName}`}
            >
                {label}
            </span>
            <input
...
export function SelectInput({
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            {isOpen && !disabled && (
                <div className="absolute left-0 right-0 top-12 z-30 overflow-hidden rounded-lg border border-gray-600 bg-gray-800 p-1 shadow-xl">
                    {options.map((option) => (
                        <button
                            key={option.value}
                            type="button"
...
export function CharacterNameInput({
    ...
    return (
        <div>
            ...
            {draftHint && (
                <div
                    className="mt-1 truncate text-xs text-amber-100"
                    title={draftHint}
                >
                    {draftHint}
                </div>
